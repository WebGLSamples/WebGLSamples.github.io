...
static long long RANDOM_RANGE_ = 4294967296;
	...
	template <typename T>
	void mulMatrixMatrix4(vector<T>& dst, vector<T>& a, vector<T>& b) {
		T a00 = a[0];
		T a01 = a[1];
		T a02 = a[2];
		T a03 = a[3];
	...
	template <typename T>
	void inverse4(vector<T>& dst, vector<T>& m) {
		T m00 = m[0 * 4 + 0];
		T m01 = m[0 * 4 + 1];
		T m02 = m[0 * 4 + 2];
		T m03 = m[0 * 4 + 3];
	...
	template <typename T>
	void transpose4(vector<T>& dst, vector<T>& m) {
		if (dst == m) {
			T t;
	...
	template <typename T>
	void frustum(vector<T>& dst, T left, T right, T bottom, T top, T near_, T far_) {
		T dx = right - left;
		T dy = top - bottom;
		T dz = near_ - far_;
	...
	template <typename T>
	void getAxis(vector<T>& dst, vector<T>& m, int axis) {
		int off = axis * 4;
		dst[0] = m[off + 0];
		dst[1] = m[off + 1];
		dst[2] = m[off + 2];
	...
	template <typename T>
	void mulScalarVector(T k, vector<T>& v) {
		for (auto& value : v) {
			value = value * k;
		}
	}
	...
	template <typename T>
	void addVector(vector<T>& dst, const vector<T>& a, const vector<T>& b) {
	...
	template <typename T>
	void normalize(vector<T>& dst, vector<T>& a) {
		T n = 0.0;
	...
	template <typename T>
	void subVector(vector<T>& dst, vector<T>& a, vector<T>& b) {
		size_t aLength = a.size();
		for (size_t i = 0; i < aLength; ++i) {
			dst[i] = a[i] - b[i];
		}
	...
	template <typename T>
	void cross(vector<T>& dst, vector<T>& a, vector<T>& b) {
	...
	template <typename T>
	void cameraLookAt(vector<T> & dst, vector<T> & eye, vector<T>& target, vector<T> & up)
	{
		vector<T> t0(3), t1(3), t2(3);
	...
	template <typename T>
	void translation(vector<T>& dst, vector<T>& v) {
		dst[0] = 1;
		dst[1] = 0;
		dst[2] = 0;
		dst[3] = 0;
	...
	template <typename T>
	void translate(vector<T>& m, vector<T>& v) {
		T v0 = v[0];
		T v1 = v[1];
		T v2 = v[2];
		T m00 = m[0];
