...
function polygonize(grid, isolevel, verts) {
  ...
  if (bits & 1)    vertlist[0]  = VertexInterp(isolevel, grid.p[0], grid.p[1], grid.val[0], grid.val[1], 0);
  if (bits & 2)    vertlist[1]  = VertexInterp(isolevel, grid.p[1], grid.p[2], grid.val[1], grid.val[2], 1);
  if (bits & 4)    vertlist[2]  = VertexInterp(isolevel, grid.p[2], grid.p[3], grid.val[2], grid.val[3], 2);
  if (bits & 8)    vertlist[3]  = VertexInterp(isolevel, grid.p[3], grid.p[0], grid.val[3], grid.val[0], 3);
  if (bits & 16)   vertlist[4]  = VertexInterp(isolevel, grid.p[4], grid.p[5], grid.val[4], grid.val[5], 4);
  if (bits & 32)   vertlist[5]  = VertexInterp(isolevel, grid.p[5], grid.p[6], grid.val[5], grid.val[6], 5);
  if (bits & 64)   vertlist[6]  = VertexInterp(isolevel, grid.p[6], grid.p[7], grid.val[6], grid.val[7], 6);
  if (bits & 128)  vertlist[7]  = VertexInterp(isolevel, grid.p[7], grid.p[4], grid.val[7], grid.val[4], 7);
  if (bits & 256)  vertlist[8]  = VertexInterp(isolevel, grid.p[0], grid.p[4], grid.val[0], grid.val[4], 8);
  if (bits & 512)  vertlist[9]  = VertexInterp(isolevel, grid.p[1], grid.p[5], grid.val[1], grid.val[5], 9);
  if (bits & 1024) vertlist[10] = VertexInterp(isolevel, grid.p[2], grid.p[6], grid.val[2], grid.val[6], 10);
  if (bits & 2048) vertlist[11] = VertexInterp(isolevel, grid.p[3], grid.p[7], grid.val[3], grid.val[7], 11);
  ...
  while (triTable[cubeindex + i] != -1) {
    verts[i + 0] = vertlist[triTable[cubeindex + i + 0]];
    verts[i + 1] = vertlist[triTable[cubeindex + i + 1]];
    verts[i + 2] = vertlist[triTable[cubeindex + i + 2]];
    i += 3;
    ntriang++;
  }
  return ntriang;
...
function MarchingCubesEffect() {
  ...
  this.render = function(framebuffer, time, global_time) {
      ...
      for (var z = 0; z < size; z++) {
        ...
        for (var y = 0; y < size; y++) {
          ...
          for (var x = 0; x < size; x++) {
            ...
            var val = 1.0/(0.15 + fx * fx + ffy * ffy + fz * fz) * 0.1
    ...
    // window.console.log("Tri cnt: " + tricount)
    // Draw a simple triangle sticking out of the cube, to show off the immediate mode simulator.
    for (var i = -100; i < 100; i++) {
     // imm.posv([0, 0, i]); imm.next();
     // imm.posv([1, 1, i]); imm.next();
     // imm.posv([1, 0, i]); imm.next();
    }
    imm.end()
  }
}
