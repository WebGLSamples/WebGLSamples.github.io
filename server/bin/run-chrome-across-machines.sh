...
function doit {
  ssh -x $GUSER@$1 "export DISPLAY=:0.0 ; ~/chrome/chrome-linux/chrome --kiosk --no-first-run --enable-webl --enable-accelerated-compositing http://$GDOMAIN/$GURL{\\\"net\\\":{\\\"ui\\\":1,\\\"sync\\\":true,\\\"slave\\\":$4,\\\"rotYMult\\\":$2\,\\\"port\\\":$GPORT\,\\\"msg\\\":\\\"$5\\\"}}" &
}
