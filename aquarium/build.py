...
def FixTexturePath(m_ob):
  ...
  if p.startswith("file:///"):
    ...
    if new_path:
      p = new_path
  return "<init_from>file:///%s</init_from>" % p
...
def GetJSObjectByTypeAndId(js, type, id):
  """."""
...
def ConvertColladaToJS(builder, input_file, output_file, temp_path, pretty):
  """."""
  if builder.ShouldBuild([input_file], [output_file]):
...
def ConvertMayaFile(builder, input_file, temp_path, out_path, pretty):
  ...
  ConvertColladaToJS(builder, collada_file, js_file, temp_path, pretty)
